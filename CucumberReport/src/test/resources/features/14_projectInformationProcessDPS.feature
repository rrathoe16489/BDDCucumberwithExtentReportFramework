#Author: RameshRathod
@projectInformation
Feature: Project Information as a DPS Process Role
  ...
  Scenario Outline: Validate Process Section
    ...
    Then validate check mark next to process section is display
    Then validate Top ProjectInformation header is completed with cheked mark
    Then closing the browser

    Examples: 
      | processDataSoftware | plantCapacity | fieldName          | alphabetinnumeric |
      | Test                | TestAuto      | Testing Automation | Testing           |
